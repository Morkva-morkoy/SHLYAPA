...
names = {'dasd': 879796568, 'мамe': 2139749, 'роме': 617562550, 'мне': 370300823, 'духи': 310799106, 'витя': 429372253, 'вове': 97233590} 

list_months = ['bruh', 'январь', 'февраль', 'март', 'апрель', 'май', 'июнь', 'июль', 'август', 'сентябрь', 'октябрь', 'ноябрь', 'декабрь']
r = sr.Recognizer()
...
def execute_cmd(cmd, voice):
  ...
  if cmd == 'cdate':
    ...
    webbrowser.open_new_tab('https://www.google.com/search?q={}'.format('+'.join(str(voice).replace('шляпа найди', '').split())))
  elif cmd == 'sublime':
    voice_for_apps = voice.split()
    voice_for_apps = voice_for_apps[2:]
    voice_for_apps = ' '.join(voice_for_apps)
    os.startfile(('C:\\Users\\SHLYAPA\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\{}').format(voice_for_apps))
